...
Hook.prototype.stop = function (callback) {  
  ...
  if (self.server) {
    ...
    self.conn.once('close', function(err) {
        delete self.conn;
    ...
    callback(Error('Nothing to stop'));
  }
};
    ...
    self.server = dnode(function (client, conn) {
      ...
      self.onAny(function (data, callback) {
        ...
        if (client.message && (parts[0] !== client.name)) {
          ...
          client.hasEvent(client.name + DELIMITER + parts, function (err, send) {
            if (!send) {
              //
